...
Hvis man vil starte og stoppe spark med de medfølgende `start-all.sh`og `stop-all.sh` skal man duplikere følgende (bare uden `export`) fra `.bashrc` over i bunden af `~/BigData/spark-z.z.z-bin-without-hadoop/conf/spark-env.sh`:
...
Desværre er der navnesammenfald mellem *hadoops* og *sparks* udgaver af `start-all.sh`og `stop-all.sh`. Så de skal kaldes med 3fuld sti, hvis man vil være sikker.
